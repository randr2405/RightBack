...
const advanceFormation = (state, flow, elapsed, duration, frozen) => {
  ...
  if (
    state.weights.every(
      (value, i) => Math.abs(value - goal[i]) < 0.0001 && Math.abs(state.velocity[i]) < 0.001
    )
  ) {
    state.weights = goal;
    state.velocity.fill(0);
...
const advancePointer = (pointer, elapsed) => {
  ...
  pointer.presenceVelocity =
    (pointer.presenceVelocity - presenceResponse * presenceMomentum * elapsed) * presenceDecay;
...
const GLYPHS = array<vec2u, ${ASCII_GLYPHS.length}>(
  ${ASCII_GLYPHS.map(
    (rows) =>
      `vec2u(${rows.slice(0, 4).reduce((sum, row, i) => sum + row * 2 ** (i * 5), 0)}u, ${rows
        .slice(4)
        .reduce((sum, row, i) => sum + row * 2 ** (i * 5), 0)}u)`
  ).join(",\n  ")}
);
...
const createRenderGraph = (gpu, outputSize, bloomSize = resolveBloomSize(outputSize)) => {
  const viewParams = uniforms(gpu, {
    ...
    environment: [0, 0, 0, 0],
    baseColor: [220 / 255, 38 / 255, 38 / 255, 1],
    highlightColor: mixColor([239 / 255, 68 / 255, 68 / 255, 1], [1, 1, 1, 1], MATERIAL_PRESETS.pearl.highlightMix),
    accentColor: [220 / 255, 38 / 255, 38 / 255, 1],
  });
  ...
  const bloomEffect = effect(gpu, BLOOM_SHADER, {
    ...
    set: { sceneTexture: sceneTarget, sceneSampler: linearSampler, post: postParams },
  });
  ...
  const finishEffect = effect(gpu, FINISH_SHADER, {
    ...
    set: { sceneTexture: sceneTarget, bloomTexture: bloomTarget, linearSampler, post: postParams },
  });
...
const PIXEL_SWAP_RIPPLE_TAIL = 1.8;
